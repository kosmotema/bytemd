...
packages.forEach((p) => {
  ...
  pkg.repository = {
    type: 'git',
    url: 'https://github.com/kosmotema/bytemd.git',
  ...
  (match, p1, offset, string) => {
    const content = plugins
      .map((p) => {
        ...
        const name = p.split('-').slice(1).join('-')
        const badge = `[![npm](https://img.shields.io/npm/v/@kosmotema/bytemd-plugin-${name}.svg?label=)](https://npm.im/@kosmotema/bytemd-plugin-${name})`
        const desc = _.upperFirst(
          pkg.description.replace('ByteMD plugin to ', '')
        )
        return `| [@kosmotema/bytemd-plugin-${name}](https://github.com/kosmotema/bytemd/tree/main/packages/plugin-${name}) | ${badge} | ${desc} |`
      })
      .filter((x) => x)
      .join('\n')
